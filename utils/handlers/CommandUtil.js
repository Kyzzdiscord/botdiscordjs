...
module.exports = async (client) => {
    (await pGlob(`${process.cwd()}/commands/*/*.js`)).map(async cmdFile => {
          ...
          if(!cmd.name) return Logger.warn(`Commande non chargée: la commande n'a pas de nom↓\n Fichier -> ${cmdFile}`)

          if(!cmd.description && cmd.type != 'USER') return Logger.warn(`Commande non chargée: la commande n'a pas de description ↓\n Fichier -> ${cmdFile}`)

          if(!cmd.category) return Logger.warn(` Commande non chargée: Pas de catégorie -> ${cmdFile}\ `)
          ...
          cmd.permissions.forEach(permission => {
            ...
            return Logger.typo(` Commande non chargée : une erreur de typo sur la permission ${permission}\n Fichier -> ${cmdFile}\ `)
        }

          });
        client.commands.set(cmd.name, cmd);
        Logger.command(`${cmd.name} a été chargée`);
    });
};
